...
class BaseLLM:
    ...
    def generate(self, prompt: str) -> str:
        ...
        """
        answer = self.batched_generate([prompt])
    ...
    def batched_generate(
        ...
        #formatted_prompts = self.format_prompt(prompts[0])

        # Tokenize prompts with padding (align on right) and get attention mask
        #inputs = self.tokenizer(formatted_prompts, padding=True, return_tensors="pt")
        ...
        for i in range(0, len(decoded), n_return):
            ans = decoded[i : i + n_return][0]
            #parsed_ans = self.parse_answer(ans[0])
            grouped.append(ans)
