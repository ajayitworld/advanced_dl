...
class PatchAutoEncoder(torch.nn.Module, PatchAutoEncoderBase):
    ...
    class PatchEncoder(torch.nn.Module):
        ...
        def __init__(self, patch_size: int, latent_dim: int, bottleneck: int):
            super().__init__()
            # Initial patchification and embedding
            self.patchify = PatchifyLinear(patch_size, latent_dim)
            # Intermediate convolution with GeLU for patch interactions
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            # Patchify and embed
            x = self.patchify(x)
            # Convert to channels-first for convolution
            x = hwc_to_chw(x)
    ...
    class PatchDecoder(torch.nn.Module):
        def __init__(self, patch_size: int, latent_dim: int, bottleneck: int):
            super().__init__()
            # Convolution to expand features back to latent_dim
            self.conv1 = torch.nn.Conv2d(bottleneck, latent_dim, kernel_size=3, padding=1)
            self.gelu = torch.nn.GELU()
            # Final unpatchification to reconstruct the image
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        # No additional loss terms needed for basic autoencoder
